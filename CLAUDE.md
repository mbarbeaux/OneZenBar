...
- **Widget signing**: the CI msixbundle is signed with the self-signed dev certificate when the `SIGNING_CERTIFICATE_BASE64` / `SIGNING_CERTIFICATE_PASSWORD` secrets exist (otherwise unsigned — also the case on Dependabot PRs, which cannot read secrets). The certificate Subject MUST equal the manifest Publisher (`CN=Michael Barbeaux`); installing the bundle requires the public `.cer` in the target machine's `LocalMachine\TrustedPeople` store. Self-signed = dev/sideload only; Store submission re-signs with Microsoft's certificate.
