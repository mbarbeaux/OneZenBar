...
- CI workflows: `build.yml` (push to `feature/**`, PRs to `develop`/`main`) and `release.yml` (push to `release/**`) run the same quality gate: `dotnet format --verify-no-changes --severity error`, build, tests.
