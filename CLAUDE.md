...
- CI workflows: `build.yml` (push to `feature/**`, PRs to `develop`/`main`), `develop.yml` (push to `develop`) and `release.yml` (push to `release/**`) run the same quality gate: `dotnet format --verify-no-changes --severity error`, build, tests.
