...
- The fast-forward push uses the `FAST_FORWARD_PAT` secret (fine-grained PAT, Contents read/write) — the default `GITHUB_TOKEN` would not trigger `main.yml`/`develop.yml` after the merge.
